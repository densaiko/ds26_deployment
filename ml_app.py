...
def run_ml_app():
    ...
    recruitment = st.selectbox("Recruitment Channel", ["referred", "sourcing", "other"])
    ...
    pred_probability_score = {'Promoted':round(prediction_probability[0][1]*100,4),
                                'Not Promoted':round(prediction_probability[0][0]*100,4)}

    if prediction == 1:
        st.success("Congratulation, you got promotion from us")
        st.write(pred_probability_score)

    elif prediction == 0:
        st.warning("You need to work harder than before")
        st.write(pred_probability_score)
